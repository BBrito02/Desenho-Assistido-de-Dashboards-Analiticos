...
As etapas consideradas no planeamento do desenvolvimento do \textit{Dashboard Designer} foram organizadas de forma sequencial, desde a definição de requisitos até à validação final da ferramenta. Em concreto, o processo foi composto pelas seguintes fases:

\begin{enumerate}
    \item \textbf{Levantamento de requisitos}: numa fase inicial foram consolidadas as necessidades funcionais e requisitos de usabilidade da ferramenta. O objetivo foi definir, de forma estruturada, o que o sistema deveria suportar e quais as funcionalidades essenciais para responder ao problema abordado na dissertação.
    
    \item \textbf{Meta-modelo e protótipos}: de seguida, foi definida a estrutura conceptual do domínio e foram desenvolvidos protótipos iniciais para orientar o desenho da interface. Esta fase permitiu validar a organização dos elementos principais do editor e assegurar coerência com o meta-modelo apresentado na Sec.~\ref{sec:meta_modelo}.
    
    \item \textbf{Implementação visual}: após a definição conceptual, foi construída a base visual do editor, incluindo o \textit{canvas}, representação de nós e menus principais, alinhados com os protótipos. O resultado desta fase foi uma interface funcional no \textit{Editor Mode} ao nível visual, permitindo já interação básica com o ambiente.
    
    \item \textbf{Implementação lógica}: nesta fase foram transferidas para a aplicação as regras e restrições do meta-modelo, incluindo atributos por nó, regras de ligação, dependências e validações necessárias para garantir coerência do grafo. O resultado foi uma estrutura semântica operacional, permitindo que o protótipo fosse mais do que um desenho visual e passasse a refletir regras do domínio.
    
    \item \textbf{Funcionalidades diferenciadoras}: com a base visual e lógica estabilizada, foram implementadas funcionalidades orientadas ao suporte do ciclo iterativo e à colaboração, com destaque para o \textit{Review Mode} e mecanismos de recolha de \textit{feedback}. Esta fase correspondeu à introdução das capacidades que distinguem a ferramenta de um editor genérico de grafos.
    
    \item \textbf{Validação e testes}: por fim, foi verificada a consistência e robustez da ferramenta através de testes manuais e validações contínuas baseadas na criação de protótipos representativos. Esta etapa permitiu identificar falhas, corrigir comportamentos inesperados e consolidar o sistema para a fase de avaliação com utilizadores.
\end{enumerate}

O levantamento de requisitos permitiu consolidar as funcionalidades que a ferramenta deve suportar, assegurando que o desenvolvimento permaneceu alinhado com o objetivo central da dissertação: apoiar a conceção, prototipagem e revisão de \textit{dashboards} interativos. De seguida, apresentam-se os requisitos funcionais do \textit{Dashboard Designer}, cujo detalhe de funcionamento é aprofundado no Capítulo~\ref{cha:arquitetura_implementacao}.
...
A inclusão desta componente torna-se fundamental uma vez que, em muitos cenários reais, um único \textit{dashboard} não é suficiente para suportar todo o processo de análise. À medida que o nível de detalhe aumenta, é comum organizar a análise em múltiplos \textit{dashboards} interligados, muitas vezes separando diferentes tipos de análise por diferentes tipos de \textit{dashboards}. Nestes casos, o utilizador necessita de mecanismos explícitos para mudar de contexto analítico de forma controlada, quer navegando para outros \textit{dashboards} do mesmo conjunto, quer acedendo a fontes externas de informação que complementem a análise. Assim, a componente de navegação permite modelar esta transição entre contextos e documentar, como e quando a navegação é desencadeada no protótipo.
...
\begin{itemize}
    \item \textbf{Dashboard Nav} que representa a navegação entre \textit{dashboards};
...
\begin{itemize}
  ...
  Como alternativa, poderia ser considerado o \textit{Vue}, no entanto, o \textit{React} foi escolhido pela maturidade do ecossistema, ampla adoção e disponibilidade de bibliotecas compatíveis com interfaces altamente interativas.
  ...
  Como alternativas, poderia ser utilizado \textit{JavaScript}, ainda assim, o \textit{TypeScript} foi escolhido para reduzir erros durante o desenvolvimento e tornar mais segura a evolução do modelo de dados da aplicação.
  ...
  Como alternativa, poderia ser utilizada uma configuração baseada em \textit{Webpack}, no entanto, o \textit{Vite} foi escolhido por privilegiar tempos de arranque reduzidos e um ciclo de desenvolvimento mais rápido.
  ...
  Como alternativa, poderia ser utilizado o \textit{Bootstrap}, ainda assim, o \textit{Tailwind CSS} foi escolhido por permitir maior controlo e consistência visual com baixo custo de manutenção, sem depender de componentes pré-definidos.
\end{itemize}

\begin{figure}[htbp]
  \includegraphics[width=\textwidth]{libraries}
  \centering
  \caption{Bibliotecas principais utilizadas.}
  \label{fig:libraries}
\end{figure}
