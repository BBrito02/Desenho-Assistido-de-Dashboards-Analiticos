...
Um requisito fundamental do \textit{Dashboard Designer} é permitir que os projetos desenvolvidos sejam preservados e partilhados entre diferentes utilizadores. Para esse efeito, o sistema separa claramente \textbf{dados estruturais} (grafo, metadados e configuração do editor) de \textbf{recursos pesados} (imagens associadas a \textit{screenshots}). A informação estrutural do projeto é guardada num formato JSON e inclui, para além das propriedades semânticas de cada elemento, a informação de \textbf{layout do \textit{canvas}}, nomeadamente a \textbf{posição} de cada nó e as suas \textbf{dimensões}. Adicionalmente, são persistidos parâmetros de contexto do editor, como a área visível do \textit{canvas} (e.g., valores de \textit{pan} e \textit{zoom}), permitindo restaurar a vista de trabalho na reabertura do projeto. Já as imagens associadas aos \textit{dashboards} são armazenadas localmente no navegador através do \textit{IndexedDB}, permitindo gerir estes recursos de forma eficiente sem sobrecarregar a informação principal do projeto. Esta decisão melhora a responsividade do editor e simplifica o ciclo de edição e revisão, especialmente quando o utilizador manipula múltiplos elementos com suporte visual.

De forma a permitir que os protótipos possam ser partilhados e reutilizados, o \textit{Dashboard Designer} utiliza um formato próprio de ficheiro com a extensão \textbf{\texttt{.dashboard}}. Este ficheiro corresponde a um \textbf{arquivo comprimido} que reúne, num único ficheiro, toda a informação necessária para voltar a abrir um protótipo. Em particular, inclui um ficheiro \texttt{dashboard.json} com a descrição estrutural do projeto, contendo a lista de nós e arestas, os respetivos metadados, bem como informação de \textbf{layout} do \textit{canvas} (posição e dimensões dos nós) e parâmetros do editor (como a área visível no \textit{canvas} e o nível de \textit{zoom}). Quando existem recursos associados, como imagens de \textit{screenshots}, estes são incluídos no mesmo pacote numa pasta própria (\texttt{images/}), sendo referenciados a partir do \texttt{dashboard.json} por identificadores. Assim, no processo de exportação o sistema armazena a estrutura e os seus recursos num ficheiro único, e no processo de importação executa o inverso, reconstruindo o grafo e restaurando os recursos necessários para que o protótipo possa ser visualizado e editado novamente.

Com a arquitetura do sistema definida, as secções seguintes descrevem as funcionalidades da aplicação em detalhe. A Secção~\ref{sec:cap4_pagina_inicial} descreve a \textit{Página Inicial}, onde o utilizador pode escolher entre iniciar um novo projeto, carregar um projeto existente ou explorar exemplos de protótipos desenvolvidos. A Secção~\ref{sec:cap4_editor_mode} aprofunda o \textit{Modo de Edição}, descrevendo o ambiente de edição, criação/manipulação de nós e o processo de configuração semântica do protótipo. A Secção~\ref{sec:cap4_review_mode} descreve o \textit{Modo de Recolha de Feedback}, dedicado à análise e recolha de \textit{feedback} contextualizado diretamente sobre elementos do \textit{dashboard}, suportando iteração e colaboração dentro da própria ferramenta.

\section{Página Inicial}
\label{sec:cap4_pagina_inicial}

A \textbf{Página Inicial} do \textit{Dashboard Designer} é apresentada ao utilizador assim que a aplicação é aberta. Esta página tem como principal objetivo fornecer uma interface clara e intuitiva, permitindo que o utilizador escolha rapidamente que ação deseja efetuar antes de iniciar o processo de edição ou revisão de um \textit{dashboard}. A página inicial funciona como um ponto de partida, oferecendo diferentes opções que facilitam o fluxo de trabalho do utilizador, ajudando-o a decidir se quer começar um novo projeto, carregar um projeto existente ou explorar exemplos de protótipos já desenvolvidos.

De seguida, são apresentadas as diferentes opções que a página inicial disponibiliza ao utilizador:
\begin{itemize}
    \item \textbf{\textit{Blank Project} (Novo Projeto)}: Ao selecionar esta opção, o utilizador inicia um novo projeto em branco. Esta opção é ideal para quem deseja começar do zero, criando um protótipo sem qualquer estrutura ou conteúdo prévio. O \textit{Dashboard Designer} abre uma área de trabalho limpa (\textit{canvas}), pronta para a adição de nós, interações e configurações, permitindo que o utilizador construa o seu \textit{dashboard} a partir de uma base vazia.
    
    \item \textbf{\textit{Load Project} (Carregar Projeto)}: Esta opção permite ao utilizador carregar um projeto previamente salvo. Quando o utilizador seleciona esta opção, a aplicação solicita a seleção de um ficheiro \texttt{.dashboard} guardado localmente. Uma vez carregado, o \textit{Dashboard Designer} restaura o projeto, mantendo todos os nós, interações, layouts e dados associados, permitindo ao utilizador retomar o estado do projeto.
    
    \item \textbf{\textit{Load Examples} (Carregar Exemplos)}: Esta funcionalidade oferece ao utilizador a opção de explorar exemplos de protótipos de \textit{dashboards} já desenvolvidos. Através desta opção, o utilizador pode visualizar diferentes tipos de protótipos e entender melhor as funcionalidades e possibilidades da ferramenta. Estes exemplos servem como inspiração, permitindo ao utilizador ver como as diferentes interações, visualizações e estruturas podem ser implementadas no \textit{Dashboard Designer}. Os exemplos fornecidos englobam todas as funcionalidades que a plataforma suporta, servindo como uma visão abrangente das capacidades do \textit{Dashboard Designer}.
\end{itemize}

A interface da página inicial foi projetada para ser simples e objetiva. Esta abordagem oferece uma experiência de uso eficiente e sem complicações. A Figura~\ref{fig:homePage} ilustra a página inicial do \textit{Dashboard Designer}, destacando as opções disponíveis e facilitando a interação do utilizador com a aplicação.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=\textwidth]{homePage}
    \caption{Página Inicial do \textit{Dashboard Designer}.}
    \label{fig:homePage}
\end{figure}
...
Para tornar mais evidente o conjunto de controlos disponibilizados no \textit{canvas}, a Figura~\ref{fig:cap4_canvas_controls} apresenta um recorte da interface onde se destacam as opções de navegação e interação do editor. Estes controlos incluem operações de \textit{zoom in/out} e \textit{pan}, a possibilidade de bloquear a interatividade do \textit{canvas} (\textit{lock}), a alternância para o modo de seleção múltipla (\textit{lasso mode}) e a opção de ocultar/mostrar as ligações de interação (\textit{hide interactions}), permitindo reduzir ruído visual em protótipos mais complexos.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.55\textwidth]{controls}
    \caption{Controlos do \textit{canvas}.}
    \label{fig:cap4_canvas_controls}
\end{figure}

Para além do \textit{canvas}, a aplicação disponibiliza elementos adicionais na interface que suportam o processo de edição. Em particular, o \textit{SideMenu}, localizado no lado esquerdo da interface, apresenta os componentes disponíveis para construção do protótipo (por exemplo, visualizações, filtros e \textit{placeholders}), permitindo iniciar a criação de nós através de uma abordagem \textit{drag-and-drop}. Adicionalmente, a zona superior do ecrã disponibiliza controlos de produtividade, como a opção de retornar para a página inicial, \textit{undo/redo}, e a possibilidade de salvar o protótipo. Por fim, é disponibilizado um seletor explícito de modo (\textit{Editor}/\textit{Review}) que permite alternar entre edição e revisão, mantendo o \textit{canvas} como elemento central do processo de prototipagem.

A Figura~\ref{fig:cap4_editor_layout} apresenta um exemplo do \textit{Dashboard Designer} em \textit{Modo de Edição}, com um protótipo desenvolvido no \textit{canvas}. Esta imagem ilustra a disposição dos principais elementos da interface, como o \textit{SideMenu}, os controlos de navegação e a interação com os componentes, fornecendo uma visão clara do ambiente de trabalho utilizado para a criação e edição do protótipo.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{editorMode}
    \caption{Interface do \textit{Dashboard Designer} em \textit{Modo de Edição}, com um protótipo desenvolvido no \textit{canvas}.}
    \label{fig:cap4_editor_layout}
\end{figure}
...
Durante a ação de \textit{drag-and-drop}, a aplicação utiliza um \textit{DragOverlay} para apresentar uma pré-visualização do elemento selecionado, aumentando a previsibilidade da ação e tornando mais claro para o utilizador o componente que será inserido no \textit{canvas}. Esta pré-visualização é materializada pelo componente \textit{NodeGhost}, que representa graficamente o tipo de nó selecionado. Desta forma, o utilizador mantém uma referência visual consistente do nó a ser inserido no canvas.
...
Para garantir que a inserção de nós é consistente em contextos hierárquicos, a aplicação identifica automaticamente o contentor mais adequado durante o gesto de \textit{drag-and-drop}. Em particular, quando o utilizador posiciona um componente sobre uma zona onde existem múltiplos contentores sobrepostos, a aplicação privilegia o contentor mais específico, isto é, o nó que se encontra mais ``interno'' na hierarquia. Por exemplo, ao arrastar um \textit{Filtro} para uma área onde existe uma \textit{Visualização} inserida no interior de um \textit{Dashboard}, a aplicação associa o \textit{Filtro} à \textit{Visualização} (desde que a inserção seja semanticamente válida), em vez de o inserir diretamente ao nível do \textit{Dashboard}. Desta forma, evita-se que o nó seja colocado num nível hierárquico incorreto quando o objetivo do utilizador era associá-lo ao contexto específico da \textit{Visualização}. Esta estratégia reduz ambiguidades durante a inserção e permite construir protótipos com vários níveis de hierarquia de forma previsível, assegurando que o nó é associado ao contexto pretendido pelo utilizador.
...
\begin{itemize}
    \item \textbf{\texttt{ComponentPopup}}: suporta a criação contextual de novos nós a partir de um nó contentor. Este \textit{popup} apresenta apenas os tipos de componentes compatíveis com o nó selecionado, orientando o utilizador para estruturas válidas e reduzindo erros semânticos durante a edição.
    ...
    \item \textbf{\texttt{VisualVariablePopup}}: suporta a definição de variáveis visuais associadas às representações (por exemplo, codificações visuais usadas para distinguir valores ou categorias), promovendo consistência na forma como os dados são codificados visualmente (Figura~\ref{fig:cap4_popups_graphtype_visualvar_b}).
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{popups/graphType}
        \caption{\textit{GraphTypePopup}.}
        \label{fig:cap4_popups_graphtype_visualvar_a}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{popups/visualVariable}
        \caption{\textit{VisualVariablePopup}.}
        \label{fig:cap4_popups_graphtype_visualvar_b}
    \end{subfigure}
    \caption{\textit{Popups} de configuração de gráficos e variáveis visuais.}
    \label{fig:cap4_popups_graphtype_visualvar}
\end{figure}

A configuração de tipos de gráfico e de variáveis visuais foi concebida para facilitar a extensão da ferramenta. Estas opções são geridas como catálogos internos, o que permite adicionar novos tipos de gráfico ou novas variáveis visuais sem alterar o fluxo de edição dos \textit{popups}. Apenas é necessário atualizar as definições internas do sistema para que as novas opções fiquem disponíveis automaticamente nos \textit{popups} correspondentes. Esta abordagem segue o mesmo princípio aplicado noutras partes do sistema, como a adição de novos tipos de nós e respetivas propriedades, permitindo evoluir o \textit{Dashboard Designer} de forma incremental e mantendo uma base de código modular, extensível e fácil de manter.
...
A transição para o \textit{Review Mode} implica uma alteração deliberada do comportamento do editor, com o objetivo de separar claramente duas fases distintas do processo: a fase de construção e prototipagem; e a fase de análise e recolha de \textit{feedback}. Para garantir que a revisão decorre de forma controlada, o \textit{Dashboard Designer} introduz um bloqueio explícito das funcionalidades de edição, reduzindo o risco de alterações acidentais ao protótipo e reforçando que o foco passa a estar na inspeção e recolha de \textit{feedback} de forma estruturada.

A Figura~\ref{fig:cap4_review_mode_locked} apresenta uma visão global da aplicação em \textit{Review Mode}, evidenciando a transição a partir do \textit{Editor Mode} e a adaptação da interface para um contexto de análise. Nesta vista, é possível observar o \textit{canvas} no \textit{Review Mode}, com ênfase na estrutura da interface. As funcionalidades de edição, antes disponibilizadas no \textit{SideMenu} no \textit{Editor Mode}, são substituídas por uma listagem global dos comentários registados no sistema, orientando o utilizador para a navegação e consulta do \textit{feedback} existente, reforçando que o foco passa a estar na inspeção do modelo e na recolha de \textit{feedback}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{reviewInterface}
    \caption{Interface do \textit{Dashboard Designer} em \textit{Review Mode}}
    \label{fig:cap4_review_mode_locked}
\end{figure}
...
O fluxo de recolha de \textit{feedback} é orientado pela seleção no \textit{canvas}. Quando o utilizador seleciona um nó ou uma aresta no \textit{Review Mode}, o menu contextual do elemento (\textit{ComponentsMenu}) deixa de apresentar opções de edição e passa a disponibilizar um formulário de comentário (\textit{ReviewMenu}). Neste menu, além das opções de comentário, também é apresentada uma lista de interações associadas ao nó selecionado, caso este possua alguma interação configurada. Esta funcionalidade permite que o utilizador, ao mesmo tempo em que fornece \textit{feedback} sobre um componente, também visualize as interações que estão vinculadas ao nó, facilitando uma revisão mais completa do comportamento e das funcionalidades do protótipo.
...
De forma a tornar mais claro o processo de recolha e gestão de \textit{feedback} no sistema, foi disponibilizado um vídeo demonstrativo no Youtube\footnote{\url{https://youtu.be/-pegLHxtSBg}}. No vídeo é apresentado o fluxo completo de associação de um comentário a um nó específico no \textit{canvas}. Após a inserção do comentário no nó, são demonstradas as principais propriedades e ações disponíveis, incluindo a edição do conteúdo do comentário, a publicação de uma resposta associada e, por fim, a alteração do seu estado para \textit{resolvido}. Este exemplo evidencia como o \textit{Dashboard Designer} suporta um ciclo de revisão estruturado diretamente sobre o protótipo, mantendo o contexto do comentário associado ao elemento avaliado.
